...
export default async function SellerOrdersPage() {
  ...
  const paidOrders = orders.filter(
    (order) => order.order.payment?.status === "PAID",
  ).length;
                  ...
                  {paymentStatus !== "PAID" && (
                    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                      ...
                      <p className="mt-1 text-xs text-amber-700">
                        The seller should not process this order until
...
function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${styles[status] ?? "bg-slate-100 text-slate-700"
        }`}
    >
      {formatStatus(status)}
    </span>
